...
public class Math3d : MonoBehaviour {
    ...
    private static Transform _tempChild = null;
    private static Transform _tempParent = null;
    
    public static void Init(){
        
        _tempChild = (new GameObject("Math3d_TempChild")).transform;
        _tempParent = (new GameObject("Math3d_TempParent")).transform;
        
        _tempChild.gameObject.hideFlags = HideFlags.HideAndDontSave;
        DontDestroyOnLoad(_tempChild.gameObject);
        
        _tempParent.gameObject.hideFlags = HideFlags.HideAndDontSave;
        DontDestroyOnLoad(_tempParent.gameObject);
        
        //set the parent
        _tempChild.parent = _tempParent;
    }
    ...
    public static void TransformWithParent(out Quaternion childRotation, out Vector3 childPosition, Quaternion parentRotation, Vector3 parentPosition, Quaternion startParentRotation, Vector3 startParentPosition, Quaternion startChildRotation, Vector3 startChildPosition){
        ...
        _tempParent.rotation = startParentRotation;
        _tempParent.position = startParentPosition;
        _tempParent.localScale = Vector3.one; //to prevent scale wandering
        
        //set the child start transform
        _tempChild.rotation = startChildRotation;
        _tempChild.position = startChildPosition;
        _tempChild.localScale = Vector3.one; //to prevent scale wandering
        
        //translate and rotate the child by moving the parent
        _tempParent.rotation = parentRotation;
        _tempParent.position = parentPosition;
        
        //get the child transform
        childRotation = _tempChild.rotation;
        childPosition = _tempChild.position;
    }
